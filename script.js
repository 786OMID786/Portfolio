...
if (contactForm && formStatus) {
  ...
  contactForm.addEventListener('submit', async event => {
    ...
    const templateParams = {
      ...
      title: fields.get('subject').trim() || 'Portfolio contact message',
    ...
    try {
      ...
      const detail = typeof error?.text === 'string' ? error.text : '';
      const status = error?.status ? ` (EmailJS ${error.status})` : '';
      console.error('EmailJS could not send the contact message:', status, detail || error);
      formStatus.dataset.state = 'error';
      if (detail) {
        formStatus.textContent = `EmailJS: ${detail}${status}. Check the template recipient and make sure it uses the form fields {{email}}, {{first_name}}, and {{message}}.`;
      } else {
        formStatus.textContent = 'Could not reach EmailJS. Check your internet connection and make sure the EmailJS service is available.';
      }
    } finally {
